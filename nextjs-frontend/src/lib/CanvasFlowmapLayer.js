...
const CanvasFlowmapLayer = L.GeoJSON.extend({
  ...
  initialize: function(geoJson, options) {
    ...
    this._animationTween = new TWEEN.Tween(this._animationProperties)
      .to( { offset: this._animationProperties.resetOffset }, this._animationPropertiesDynamic.duration)
  ...
  _screenPoint: function( latlngGetter ) {
    this._map.latLngToContainerPoint(this._wrapAroundLatLng(latlngGetter(feature)));
  },

  _drawSelectedCanvasPaths: function(animate) {
    const ctx = animate ? this._animationCanvasElement.getContext('2d') : this._canvasElement.getContext('2d');
    const originAndDestinationFieldIds = this.options.originAndDestinationFieldIds;
    ...
    for ( const feature of features ) 
    {
      ...
      const { symbol } = animate ? this.options.animatedCanvasBezierStyle : this.options.canvasBezierStyle
      const drawFunc = animate ? this._animateCanvasLineSymbol : this._applyCanvasLineSymbol
  ...
  _animateCanvasLineSymbol: function(ctx, symbolObject, screenOriginPoint, screenDestinationPoint) {
    ...
    ctx.shadowBlur = symbolObject.shadowBlur;
    ctx.shadowColor = symbolObject.shadowColor;
    ctx.setLineDash([symbolObject.lineDashOffsetSize, (this._animationProperties.resetOffset - symbolObject.lineDashOffsetSize)]);
